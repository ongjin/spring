...
public class BoardController {
	 ...
	 * 
	 * PathVariable 언제 사용할까? - 자원을 식별하는 용도 값(파라미터)
	 * 
	 * QueryString 언제 사용할까? - 필터링(검색, 정렬, 현재 페이지)
	 */
	// 게시글 상세 조회
	...
	/* @ModelAttribute("loginMember") Member loginMember */
		...
		// @@@@@@@@@@@@@@ ㄹㅇ 중요 @@@@@@@@@@@@@@@@@@@@@@@@@
		// @SesstionAttributes
		// 1) Model.addAttribute(K, V) 수행 시 K가 일치하는 값을 Request -> Session scope
	...
	public String boardInsert(Board board, @RequestParam(value = "images", required = false) List<MultipartFile> images,
		...
		/*
		 * MultipartFile이 제공하는 메소드 - getOriginalFilename() : 파일 원본명 반환 - getSize() : 파일
		 * 크기 반환 - getInputStream() : 파일에 대한 입력 스트림 - transferTo() : MultipartFile 객체에는
		 * 메모리에 저장된 파일과 연결되어 있음 해당 메소드 호출 시 연결된 메모리 파일을 디스크(HDD, SSD)로 저장
		 */
