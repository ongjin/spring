...
public class ChatWebsocketHandler extends TextWebSocketHandler {
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		// 클래스를 이용해 새 객체를 만들고, JSON 문자열을 읽어 같은 필드에 값을 대입함.
		...
		if(result > 0) {
			...
			for (WebSocketSession wss : sessions) {
				...
				// sessions에 저장된 모든 클라이언트 세션 정보에서 chatRoomNo 속성을 얻어오는 구문
				
				int chatRoomNo = (Integer)wss.getAttributes().get("chatRoomNo");
				
				// 메세지에 있는 방번호와 채팅방에 있으면서(웹소켓 연결 중) 같은 방번호를 가지고 있는 클라이언트인 경우
